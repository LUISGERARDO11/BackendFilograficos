...
const { User, Order, OrderDetail, Product, ProductVariant, Category, Customization, Badge, UserBadge, BadgeCategory, RevokedToken } = require('../../src/models/Associations');
...
jest.mock('../../src/models/Associations', () => {
  ...
  return {
    User: mockModel('User'),
    Order: { ...mockModel('Order'), addHook: jest.fn() },
...
describe('Gamification Integration Tests', () => {
  ...
  beforeAll(async () => {
    User.create.mockResolvedValue({ user_id: 1, name: 'Test User', email: 'test@example.com', phone: '1234567890', status: 'activo', user_type: 'cliente' });
    Category.create.mockResolvedValue({ category_id: 1, name: 'Electronics', active: true });
    BadgeCategory.create.mockResolvedValue({ badge_category_id: 1, name: 'General', is_active: true });
    Badge.create.mockResolvedValue({ badge_id: 7, name: 'Coleccionista', description: '3+ productos en una categoría', badge_category_id: 1, is_active: true, icon_url: 'http://example.com/icon.png', public_id: 'badge_7' });

    await sequelize.authenticate();
    await sequelize.sync({ force: true });
    const BadgeService = require('../../src/services/BadgeService');
    notificationManagerInstance = new mockNotificationManager();
    setupGamificationHooks(new BadgeService(), notificationManagerInstance);

    user = await User.create({ user_id: 1, name: 'Test User', email: 'test@example.com', phone: '1234567890', status: 'activo', user_type: 'cliente' });
    category = await Category.create({ category_id: 1, name: 'Electronics', active: true });
    await BadgeCategory.create({ badge_category_id: 1, name: 'General', is_active: true });
    badge = await Badge.create({ badge_id: 7, name: 'Coleccionista', description: '3+ productos en una categoría', badge_category_id: 1, is_active: true, icon_url: 'http://example.com/icon.png', public_id: 'badge_7' });
