...
exports.createRegulatoryDocument = async (req, res) => {
  try {
    ...
    do {
      iteration++;
      const isContentSuspicious = isSuspiciousContent(originalContent);
    ...
    if (existingDoc) {
      ...
      return res.status(200).json({
        message: `Documento actualizado a versión ${newVersion}`,
...
exports.updateRegulatoryDocument = async (req, res) => {
  try {
    ...
    // 3. Convertir el archivo .docx a texto
...
exports.getDocumentById = async (req, res) => {
  try {
    ...
    // Mapear respuesta con estado calculado
    const versions = document.DocumentVersions.map(version => {
      let status;
      if (version.deleted) {
        status = 'Eliminado';
      } else {
        status = version.active ? 'Vigente' : 'Histórico';
      }
...
exports.restoreRegulatoryDocumentVersion = async (req, res) => {
  try {
    ...
    if (latestVersion.version_id === version.version_id) {
      await DocumentVersion.update(
        { active: false },
        { where: { document_id: version.document_id, active: true } }
      );
