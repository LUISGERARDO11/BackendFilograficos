...
exports.updateBadge = [
    ...
    async (req, res) => {
        ...
        try {
            ...
            const badge = await badgeService.updateBadge(id, { name, description, badge_category_id }, req.file ? req.file.buffer : null, transaction);
            if (!badge) {
                await transaction.rollback();
...
exports.getGrantedBadgesHistory = [
    ...
    async (req, res) => {
        try {
            ...
            const { page = 1, pageSize = 10, sort, userId, badgeId, startDate, endDate } = req.query;
            const pageInt = parseInt(page);
            const pageSizeInt = parseInt(pageSize);
            ...
            if (startDate || endDate) {
                where.obtained_at = {};
                if (startDate) where.obtained_at[Op.gte] = new Date(startDate);
                if (endDate) where.obtained_at[Op.lte] = new Date(endDate + 'T23:59:59.999Z'); // Incluir todo el día
            }

            let order = [['obtained_at', 'DESC']]; // Orden predeterminado
            if (sort) {
                const sortParams = sort.split(',').map(param => param.trim().split(':'));
                // Columnas válidas para ordenar en UserBadge
            ...
            const { count, rows: history } = await badgeService.getGrantedBadgesHistory({
                where,
                order,
                page: pageInt,
                pageSize: pageSizeInt
